...
func TestPackageCLIHelpAndErrors(t *testing.T) {
	...
	if code != 0 || !strings.Contains(stdout, "Usage:") || !strings.Contains(stdout, "pi install <source> [-l]") || stderr != "" {
		t.Fatalf("code=%d stdout=%q stderr=%q", code, stdout, stderr)
	}

	code, _, stderr = runPackageCLI(t, []string{"install", "--unknown"})
	if code != 1 || !strings.Contains(stderr, `Unknown option --unknown for "install".`) ||
		!strings.Contains(stderr, `pi install <source> [-l] [--approve|--no-approve]`) {
		t.Fatalf("code=%d stderr=%q", code, stderr)
	}

	code, _, stderr = runPackageCLI(t, []string{"install"})
	if code != 1 || !strings.Contains(stderr, "Missing install source.") || !strings.Contains(stderr, "Usage: pi install <source> [-l]") {
		t.Fatalf("code=%d stderr=%q", code, stderr)
	}
...
func TestPackageCLIUpdateTargets(t *testing.T) {
	...
	// Bare update prints the skipped-extensions note, then fails on
	// self-update (deferred to WP-661).
	code, stdout, stderr = runPackageCLI(t, []string{"update"})
	if code != 1 || !strings.Contains(stdout, "Extensions are skipped. Run pi update --extensions to update extensions.") ||
		!strings.Contains(stderr, "cannot self-update") {
		t.Fatalf("code=%d stdout=%q stderr=%q", code, stdout, stderr)
	}
...
func TestConfigCLICommand(t *testing.T) {
	...
	if code != 0 || !strings.Contains(stdout, "pi config [-l]") {
		t.Fatalf("code=%d stdout=%q", code, stdout)
	}
