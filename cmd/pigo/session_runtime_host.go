...
func (session *cliPrintSession) Bind(replacement *codingagent.AgentSession) error {
	if replacement == nil {
		return fmt.Errorf("pigo: print mode replacement returned no session")
	}
...
func (session *cliPrintSession) Prompt(ctx context.Context, input any, images ...*ai.ImageContent) error {
	...
	if current == nil {
		return fmt.Errorf("pigo: print mode session is unavailable")
	}
...
func newCLISessionRuntimeHost(
	...
	if options.Manager == nil {
		return nil, fmt.Errorf("pigo: session runtime host requires a session manager")
	}
	...
	factory := func(_ context.Context, runtimeOptions codingagent.AgentSessionOptions) (*codingagent.AgentSessionResult, error) {
		...
		if manager == nil {
			return nil, fmt.Errorf("pigo: replacement runtime requires a session manager")
		}
