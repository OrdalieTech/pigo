...
import (
	...
	upstreamsync "github.com/OrdalieTech/pi-go/internal/sync"
)
...
func run(args []string) int {
	flags := flag.NewFlagSet("pisync", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	var config upstreamsync.Config
	var noFetch bool
	flags.StringVar(&config.Root, "repo-root", "", "pi-go repository root (auto-detected by default)")
	...
	if flags.NArg() != 0 {
		fmt.Fprintf(os.Stderr, "pisync: unexpected arguments: %v\n", flags.Args())
		return 2
	}
	if !config.Bump {
		config.DryRun = true
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "pisync: %v\n", err)
		return 1
	}
	return 0
}
