...
The extractor runs from the `.upstream/` working directory with Node 22 or newer. The Makefile installs only the pinned dev-time tools needed by registered extractors; F1 uses `tsx@4.22.1`, upstream's `partial-json@0.1.7`, and upstream's `typebox@1.1.38`, while F2 also uses upstream's `openai@6.26.0` transport. A fixture change is accepted only when the cited upstream behavior changed or a new case was deliberately added; a failing Go comparison is fixed in Go, never by weakening or hand-editing the golden.
